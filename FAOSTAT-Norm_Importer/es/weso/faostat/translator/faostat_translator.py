...
class FaostatTranslator(object):
	...
	"""
	Try to find the indicator in the list of needed indicators. In this list, the items are obtained from a config file.
	The items in that list are using the patter ITEM_CODE-ELEMENT_CODE
	To compare them, a generated code from the candidate register with this pattern.
	"""
	generated_code = str(candidate_register[TranslatorConst.ITEM_CODE])+'-'+str(candidate_register[TranslatorConst.ELEMENT_CODE])
        if generated_code in self._needed_indicators:
            return True
        return False
    ...
    def parse_register_fields(self, primitive_data):
        # TODO change this code. result is a list (not an array with fixed positions).
        # So, right now, the order of the inserts in the list is crucial (and it should not be)
        # list.insert:Inserts object obj into list at offset index (NOT A POSITION)
        result = []
        ...
        result.insert(TranslatorConst.YEAR_CODE,"") # not used value
